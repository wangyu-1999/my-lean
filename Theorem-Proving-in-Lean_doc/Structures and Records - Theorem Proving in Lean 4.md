# <a href="https://leanprover.github.io/theorem_proving_in_lean4/structures_and_records.html#structures-and-records" id="structures-and-records" class="header">Structures and Records</a>
...
Recall that a non-recursive inductive type that contains only one constructor is called a *structure* or *record*. The product type is a structure, as is the dependent product (Sigma) type. In general, whenever we define a structure `S`, we usually define *projection* functions that allow us to "destruct" each instance of `S` and retrieve the values that are stored in its fields. The functions `prod.pr1` and `prod.pr2`, which return the first and second elements of a pair, are examples of such projections.
...
## <a href="https://leanprover.github.io/theorem_proving_in_lean4/structures_and_records.html#declaring-structures" id="declaring-structures" class="header">Declaring Structures</a>
...
The dot notation is convenient not just for accessing the projections of a record, but also for applying functions defined in a namespace with the same name. Recall from the [Conjunction section](https://leanprover.github.io/theorem_proving_in_lean4/propositions_and_proofs.html#_conjunction) if `p` has type `Point`, the expression `p.foo` is interpreted as `Point.foo p`, assuming that the first non-implicit argument to `foo` has type `Point`. The expression `p.add q` is therefore shorthand for `Point.add p q` in the example below.
...
## <a href="https://leanprover.github.io/theorem_proving_in_lean4/structures_and_records.html#objects" id="objects" class="header">Objects</a>
...
## <a href="https://leanprover.github.io/theorem_proving_in_lean4/structures_and_records.html#inheritance" id="inheritance" class="header">Inheritance</a>
